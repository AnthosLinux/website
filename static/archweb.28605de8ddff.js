if (typeof $ !== "undefined" && typeof $.tablesorter !== "undefined") {
  ...
  $.tablesorter.removeParser = function (id) {
    $.tablesorter.parsers = $.grep($.tablesorter.parsers, function (ele, i) {
      return ele.id !== id;
    });
  };
...
function todolist_flag() {
  ...
  $.getJSON(link.href, function (data) {
    $(link)
      .text(data.status)
      .removeClass("complete inprogress incomplete")
      .addClass(data.css_class.toLowerCase());
    $(".results").trigger("updateCell", [
      $(link).closest("td")[0],
      false,
      null,
    ]);
...
function filter_pkgs_list(filter_ele, tbody_ele) {
  ...
  $(filter_ele + " .arch_filter")
    .add(filter_ele + " .repo_filter")
    .each(function () {
      if (!$(this).is(":checked")) {
...
function filter_signoffs() {
  var rows = $("#tbody_signoffs").children(),
    all_rows = rows;
  $("#signoffs_filter .arch_filter")
    .add("#signoffs_filter .repo_filter")
    .each(function () {
      if (!$(this).is(":checked")) {
        rows = rows.not("." + $(this).val());
      }
...
function collapseNotes(elements) {
  ...
  $(elements).each(function (idx, ele) {
    ...
    ele.append(
      '<br class="morelink-spacer"/><a class="morelink" href="#">Show More…</a>'
    );
